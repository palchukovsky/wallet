...
IMAGE_TAG_REST_SERVER = $(ORGANIZATION)/$(PRODUCT):$(TAG)
...
build: ## Build docker image from actual local sources.
	@$(call build_docker_cmd_image,rest-server,$(IMAGE_TAG_REST_SERVER))

release: ## Push docker image to the hub.
	@$(call push_docker_cmd_image,$(IMAGE_TAG_REST_SERVER))

mock: ## Generate mock interfaces for unit-tests.
	@$(call get_mock,executor,Executor)
	@$(call get_mock,accountrepo,AccountRepoTrans AccountRepo)